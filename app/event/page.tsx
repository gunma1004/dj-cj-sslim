...
// ⚠️ 대전과 청주 각각 1개씩 단독 지정된 타이틀 및 메타 디스크립션
const CITY_STATIC_SEO: Record<
  string,
  {
    title: string;
    description: string;
...
> = {
  daejeon: {
    title: `대전 마사지 단독 할인 이벤트 총정리 | ${BRAND_NAME}`,
    description: `대전 출장 마사지 단독 오픈 이벤트! 전 지역 첫 방문 10,000원 즉시 할인 혜택과 선입금 없는 100% 현장 후불제 안심 케어를 경험해보세요.`,
  },
  cheongju: {
    title: `청주 마사지 단독 할인 이벤트 총정리 | ${BRAND_NAME}`,
    description: `청주 출장 마사지 단독 오픈 이벤트! 전 지역 첫 방문 10,000원 즉시 할인 혜택과 선입금 없는 100% 현장 후불제 안심 케어를 경험해보세요.`,
  },
};

// 지역별 독립 데이터 (대전 5개 구 66개 동 / 청주 4개 구 28개 동)
...
// ⚠️ 정적 내보내기 필수 파라미터 (daejeon, cheongju)
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const seo = CITY_STATIC_SEO[region];
  const regionInfo = REGION_FULL_DATA[region];

  if (!seo || !regionInfo) return {};
...
export default async function RegionEventPage({ params }: PageProps) {
  ...
  if (!regionInfo || !seo) return notFound();
