...
const CITY_KEY = "cheongju";

export async function generateMetadata() {
  const cityInfo = CITIES_DATA[CITY_KEY];
  if (!cityInfo) return {};

  const title = "청주 스웨디시 마사지 추천 전국 타이 아로마 케어 총정리 | " + BRAND_NAME;
  const description = `${BRAND_NAME}에서 청주 출장마사지 와 스웨디시마사지를 경험하세요 전문적인 테라피스트가 고객님의 집 호텔 오피스로 직접 방문 코스 가격과 관리사 정보를 확인하고 전화 ,문자로 예약하세요`;
...
export default async function CheongjuPage() {
  const cityInfo = CITIES_DATA[CITY_KEY];
  if (!cityInfo) return notFound();

  const mainColor = "#ba8cff"; // 청주 전용 포인트 컬러
  const seoTitle = "청주 스웨디시 마사지 추천 전국 타이 아로마 케어 총정리";
  const seoDesc = `${BRAND_NAME}에서 청주 출장마사지 와 스웨디시마사지를 경험하세요 전문적인 테라피스트가 고객님의 집 호텔 오피스로 직접 방문 코스 가격과 관리사 정보를 확인하고 전화 ,문자로 예약하세요`;
